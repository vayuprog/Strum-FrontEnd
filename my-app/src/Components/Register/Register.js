...
const Register = () => {
  ...
  const handleNavigation = () => {
    // Perform the navigation to the profile route here
    window.location.href = "/"; // Replace with the appropriate route to the Profile component
  };
  ...
  useEffect(() => {
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      axios.post("http://localhost:9002/signup/", user).then((res) => {
        ...
        // Navigate to the appropriate route upon successful signup
        // For example:
        window.location.href = "/Profile"; // Redirect to the login route after successful signup
      });
    }
  }, [formErrors]);
